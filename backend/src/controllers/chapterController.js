...
async function generateChapter(req, res) {
  try {
    ...
    if (!outlineId || chapter_index == null || !title) {
      return res
        .status(400)
        .json({ error: "outlineId, chapter_index, and title are required." });
    }
    ...
    if (!chapter_notes_status || chapter_notes_status === "no") {
      ...
      await sendNotification("chapter-paused", {
        outlineId,
        chapter_index,
        title,
      });
      return res.status(202).json({
        ...
        status: "paused",
        message:
          'Chapter paused. chapter_notes_status must be set to "yes" or "no_notes_needed" to proceed.',
      });
    }
    ...
    if (chapter_notes_status === "yes") {
      if (!chapter_notes || !chapter_notes.trim()) {
        ...
        await sendNotification("chapter-awaiting-notes", {
          outlineId,
          chapter_index,
          title,
        });
        return res.status(202).json({
          ...
          status: "awaiting_notes",
          message:
            "Waiting for chapter_notes from editor (chapter_notes_status=yes).",
        });
      }
      ...
      const summary = await summarizeChapterContent({
        chapterTitle: title,
        chapterDraft: draft,
      });
      ...
      await sendNotification("chapter-ready-for-review", {
        outlineId,
        chapter_index,
        title,
      });
    ...
    res
      ...
      .json({
        error:
          'Invalid chapter_notes_status. Use "yes", "no", or "no_notes_needed".',
      });
  } catch (error) {
    console.error(error);
    ...
    res
      .status(500)
      .json({ error: error.message || "Chapter generation failed." });
  }
}
...
async function acceptChapter(req, res) {
  try {
    ...
    await sendNotification("chapter-accepted", {
      chapterId,
    ...
    res
      .status(500)
      .json({ error: error.message || "Failed to accept chapter." });
  }
}
