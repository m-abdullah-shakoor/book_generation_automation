...
function App() {
  ...
  const [statusOutlineNotes, setStatusOutlineNotes] =
    useState("no_notes_needed");
  ...
  const [status, setStatus] = useState(
    'Upload an Excel outline with "Chapter Title" and optional "Outline" columns.',
  );
  const [approvedOutlineText, setApprovedOutlineText] = useState("");
  ...
  const generateOutline = async () => {
    ...
    try {
      ...
      setApprovedOutlineText(response.data.outline.outline || "");
  ...
  const regenerateOutline = async () => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE}/api/outline/${outlineRecord.id}/regenerate`,
        {
          title: bookTitle,
          notes_on_outline_before: outlineNotesBefore,
          notes_on_outline_after: outlineNotesAfter,
  ...
  const acceptChapter = async (index) => {
    ...
    try {
      const response = await axios.post(
        `${API_BASE}/api/chapters/${chapter.id}/accept`,
      );
      setChapterRecords((prev) => ({
        ...prev,
        [index]: response.data.chapter,
  ...
  const approveOutline = async () => {
    if (!outlineRecord?.id) {
      setStatus("Generate an outline first.");
      return;
    }

    setStatus("Approving outline...");
    try {
      const response = await axios.post(
        `${API_BASE}/api/outline/${outlineRecord.id}/approve`,
        {
          approved_outline: approvedOutlineText,
        },
      );

      setOutlineRecord(response.data.outline);
      setOutlineApproved(true);
      setStatus("Outline approved. You can now generate chapters.");
    } catch (error) {
      console.error(error);
      setStatus(error.response?.data?.error || "Failed to approve outline.");
    }
  };
      ...
      <div className="container">
          ...
          {parsedOutline && (
            <div className="outline-form">
              ...
              <label>
                ...
                <textarea
                  value={outlineNotesBefore}
                  onChange={(e) => setOutlineNotesBefore(e.target.value)}
                />
              </label>
              <label>
                Outline Status
                <select
                  value={statusOutlineNotes}
                  onChange={(e) => setStatusOutlineNotes(e.target.value)}
                >
              ...
              <label>
                Notes After Outline
                <textarea
                  value={outlineNotesAfter}
                  onChange={(e) => setOutlineNotesAfter(e.target.value)}
                />
              </label>
              <div className="button-row">
                <button onClick={generateOutline}>Generate Outline</button>
                <button onClick={regenerateOutline} disabled={!outlineRecord}>
                  Regenerate Outline
                </button>
              </div>
            </div>
        ...
        {outlineRecord && (
          <section className="card">
            ...
            <p>
              <strong>Status:</strong> {outlineRecord.stage}
            </p>

            <textarea
              className="draft-box"
              value={approvedOutlineText}
              onChange={(e) => setApprovedOutlineText(e.target.value)}
              rows={18}
            />

            <div className="button-row">
              <button onClick={compileFinal}>
                Approve Outline & Continue
              </button>

              <button onClick={regenerateOutline}>
                Request Revision / Regenerate
              </button>
            </div>
          </section>
        )}

        {/* {outlineRecord && outlineApproved && parsedOutline && (
          <section className="card">
                ...
                <div key={`${chapter.title}-${index}`} className="chapter-item">
                  <div>
                    ...
                    <label>
                      ...
                      <select
                        value={chapterNotesStatus[index] || "no"}
                        onChange={(e) =>
                          setChapterNotesStatus((prev) => ({
                            ...prev,
                  ...
                  <div className="chapter-actions">
                    <button onClick={() => generateChapter(chapter, index)}>
                      Generate Chapter
                    </button>
                    {chapterRecords[index]?.id && (
                      <button onClick={() => acceptChapter(index)}>
                        Accept
                      </button>
                    )}
                  </div>
        ...
        {/* <section className="card">
          ...
          <label>
            ...
            <select
              ...
              onChange={(e) => setFinalReviewStatus(e.target.value)}
            >
              <option value="yes">yes</option>
              <option value="no_notes_needed">no_notes_needed</option>
              <option value="no">no</option>
